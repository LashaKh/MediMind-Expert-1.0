const abg = {
  ...
  analysis: {
    ...
    error: {
      ...
      service503: 'Сервис ИИ временно недоступен (503). Обычно это решается в течение нескольких минут. Пожалуйста, попробуйте снова позже.',
      rateLimit: 'Превышен лимит запросов. Пожалуйста, подождите немного и попробуйте снова.',
      quotaExhausted: 'Квота API исчерпана на сегодня. Пожалуйста, попробуйте завтра или обратитесь в поддержку.',
      geminiError: 'Ошибка сервиса анализа ИИ: {{message}}. Это, скорее всего, временно - пожалуйста, попробуйте через несколько минут.',
      unexpectedInterpretation: 'Во время интерпретации произошла непредвиденная ошибка',
      unexpectedActionPlan: 'Во время генерации плана действий произошла непредвиденная ошибка'
    },
