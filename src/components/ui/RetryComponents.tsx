...
export const ManualRetryButton: React.FC<ManualRetryProps> = ({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center px-4 py-2 border border-[var(--glass-border-medium)] rounded-md shadow-sm text-sm font-medium text-[var(--foreground-tertiary)] bg-[var(--component-card)] hover:bg-[var(--component-surface-primary)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--cardiology-accent-blue)] disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
    >
      {isRetrying ? (
        <>
          <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-[var(--foreground-secondary)]" fill="none" viewBox="0 0 24 24">
...
export const RetryIndicator: React.FC<RetryIndicatorProps> = ({
  ...
  return (
    <div className={`flex items-center text-sm text-[var(--foreground-tertiary)] ${className}`}>
