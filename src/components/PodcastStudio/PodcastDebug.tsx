...
const PodcastDebug: React.FC<PodcastDebugProps> = ({ onRefresh }) => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'info': return <Clock className="w-4 h-4 text-[var(--cardiology-accent-blue)]" />;
      default: return <Clock className="w-4 h-4 text-[var(--foreground-secondary)]" />;
    }
  };

  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-[var(--component-card)] rounded-xl border border-[var(--glass-border-light)] p-6">
        <div className="flex items-center space-x-3 mb-6">
          <div className="flex items-center justify-center w-10 h-10 bg-yellow-100 rounded-lg">
            <Bug className="w-5 h-5 text-yellow-600" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-[var(--foreground)]">Podcast System Debug</h2>
            <p className="text-[var(--foreground-tertiary)] text-sm">Tools to diagnose and fix podcast issues</p>
          </div>
        </div>
          ...
          {debugActions.map((action) => {
            ...
            return (
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="p-4 border border-[var(--glass-border-light)] rounded-lg hover:border-[var(--glass-border-medium)] hover:shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed text-left"
              >
                ...
                <p className="text-sm text-[var(--foreground-tertiary)]">{action.description}</p>
              </motion.button>
            );
          })}
        </div>
        ...
        {loading && (
          <div className="flex items-center justify-center py-8">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-[var(--foreground-tertiary)]">Running diagnostics...</span>
            </div>
          </div>
        )}
        ...
        {results.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-[var(--foreground)]">Results</h3>
              <button
                onClick={() => setResults([])}
                className="text-sm text-[var(--foreground-tertiary)] hover:text-[var(--foreground)]"
              >
                Clear
              </button>
            </div>
            ...
            <div className="space-y-3">
              {results.slice().reverse().map((result, index) => (
                <div key={index} className="bg-[var(--component-surface-primary)] rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="font-semibold text-[var(--foreground)]">{result.action}</h4>
                    <span className="text-xs text-[var(--foreground-secondary)]">{new Date(result.timestamp).toLocaleTimeString()}</span>
                  </div>
                      ...
                      result.data?.results && Array.isArray(result.data.results) ? (
                        result.data.results.map((test: { test: string; status: string; details: string }, testIndex: number) => (
                          <div key={testIndex} className="flex items-center space-x-3">
                            ...
                            <span className="font-medium text-sm text-[var(--foreground)]">{test.test}:</span>
                            <span className="text-sm text-[var(--foreground-tertiary)]">{test.details}</span>
                          </div>
                        ))
                      ) : (
                        <div className="flex items-center space-x-3">
                          <CheckCircle className="w-4 h-4 text-green-500" />
                          <span className="text-sm text-[var(--foreground-tertiary)]">Action completed successfully</span>
                        </div>
                      )
                    ) : (
                      <div className="flex items-center space-x-3">
                        ...
                        {result.details && <span className="text-sm text-[var(--foreground-secondary)]">({result.details})</span>}
                      </div>
                    )}
                  </div>
                </div>
