...
const ExpandableSection: React.FC<{ 
  ...
  return (
    <div className="border border-[var(--glass-border-light)] rounded-lg mb-2">
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full px-4 py-3 text-left flex items-center justify-between hover:bg-[var(--component-surface-primary)]"
      >
        ...
        {isExpanded ? (
          <ChevronDownIcon className="h-5 w-5 text-[var(--foreground-secondary)]" />
        ) : (
          <ChevronRightIcon className="h-5 w-5 text-[var(--foreground-secondary)]" />
        )}
      </button>
      {isExpanded && (
...
const DebugTracker: React.FC<DebugTrackerProps> = ({ debugInfo, isVisible, onToggle }) => {
    ...
    return (
      <button
        ...
        className={`
          ...
          ${hasDebugData 
            ? 'bg-gradient-to-r from-green-500 to-blue-500 text-[var(--foreground)] shadow-xl animate-pulse' 
            : 'bg-[var(--cardiology-accent-blue-dark)] text-[var(--foreground)] hover:bg-[var(--cardiology-accent-blue-dark)]'
          }
        `}
      >
        <span>{hasDebugData ? 'Comprehensive Debug Data Available' : 'Show Debug Info'}</span>
        {hasDebugData && (
          <div className="w-2 h-2 bg-[var(--component-card)] rounded-full animate-ping" />
        )}
      </button>
    );
  }
  ...
  if (!debugInfo) {
    return (
      <div className="fixed bottom-4 right-4 w-96 bg-[var(--component-card)] border border-[var(--glass-border-light)] rounded-lg shadow-lg p-4 z-50">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-[var(--foreground)]">Debug Tracker</h3>
          <button
            onClick={onToggle}
        ...
        <div className="text-center text-[var(--foreground-secondary)] py-8">
          <InformationCircleIcon className="h-12 w-12 mx-auto mb-2 text-[var(--foreground-secondary)]" />
          <p>No debug information available</p>
          <p className="text-sm text-[var(--foreground-secondary)] mt-1">Generate a podcast to see debug data</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed bottom-4 right-4 w-[600px] max-h-[80vh] bg-[var(--component-card)] border border-[var(--glass-border-light)] rounded-lg shadow-lg overflow-hidden z-50">
      <div className="flex items-center justify-between p-4 border-b border-[var(--glass-border-light)] bg-gradient-to-r from-green-50 to-blue-50">
        <h3 className="text-lg font-semibold text-[var(--foreground)]">Comprehensive 3-Step Debug Tracker</h3>
        <div className="flex items-center space-x-4">
          <span className="text-sm text-[var(--foreground-tertiary)]">Total: {totalTime}ms</span>
          <button
            onClick={onToggle}
            className="text-[var(--foreground-secondary)] hover:text-[var(--foreground-tertiary)]"
          >
        ...
        >
          <div className="space-y-2 mt-2">
            {debugInfo.step1_document_overview?.overview && (
              <>
                <div className="bg-[var(--cardiology-accent-blue-light)] p-3 rounded-md">
                  <p className="text-sm font-medium text-blue-900">
                    Primary Focus: <span className="text-[var(--cardiology-accent-blue-dark)]">{debugInfo.step1_document_overview.overview.globalAnalysis?.primaryFocus}</span>
                  </p>
                  ...
                  <details className="mt-2">
                    ...
                    <div className="mt-1 space-y-1">
                      {debugInfo.step1_document_overview.overview.documents.map((doc, i) => (
                        <div key={i} className="bg-[var(--component-surface-primary)] p-2 rounded text-xs">
                          <p className="font-medium">{doc.title}</p>
                          <p className="text-[var(--foreground-tertiary)]">Type: {doc.type} | Specialty: {doc.medicalSpecialty}</p>
                          {doc.keyTopics?.length > 0 && (
                            <p className="text-[var(--cardiology-accent-blue-dark)]">Topics: {doc.keyTopics.slice(0, 3).join(', ')}</p>
                          )}
                        </div>
                      ))}
                    </div>
              ...
              <>                
                    ...
                    <div className="mt-1 space-y-1">
                      {debugInfo.step2_content_mapping.contentMap.medicalConditions.slice(0, 5).map((condition, i) => (
                        <div key={i} className="bg-[var(--component-surface-primary)] p-2 rounded text-xs">
                          ...
                          <p className="text-[var(--foreground-tertiary)]">Type: {condition.type} | Severity: {condition.severity}</p>
                        </div>
                      ))}
                    </div>
                  </details>
                    ...
                    <div className="mt-1 space-y-1">
                      {debugInfo.step2_content_mapping.contentMap.treatmentOptions.slice(0, 5).map((treatment, i) => (
                        <div key={i} className="bg-[var(--component-surface-primary)] p-2 rounded text-xs">
                          ...
                          <p className="text-[var(--foreground-tertiary)]">Type: {treatment.type} | Efficacy: {treatment.efficacy}</p>
                        </div>
                      ))}
                    </div>
                  </details>
                ...
                {debugInfo.step2_content_mapping.contentMap.keyFindings?.length > 0 && (
                  <details className="mt-2">
                    ...
                    <ul className="text-xs text-[var(--foreground-tertiary)] mt-1 ml-4">
            ...
            {debugInfo.step3_comprehensive_outline?.outline && (
              <>
                ...
                {debugInfo.step3_comprehensive_outline.outline.chapters?.map((chapter, index) => (
                  <div key={index} className="bg-[var(--component-surface-primary)] p-2 rounded-md">
                      ...
                      <details className="mt-1">
                        ...
                        <div className="mt-1 space-y-1">
                          {chapter.segments.slice(0, 3).map((segment, i) => (
                            <div key={i} className="bg-[var(--component-card)] p-1 rounded text-xs">
                  ...
                  <details className="mt-2">
                    ...
                    <div className="mt-1 space-y-1">
                      {debugInfo.step3_comprehensive_outline.outline.citations.map((citation, i) => (
                        <div key={i} className="bg-[var(--cardiology-accent-blue-light)] p-2 rounded text-xs">
                          <p className="font-medium">{citation.title}</p>
                          <p className="text-[var(--foreground-tertiary)]">ID: {citation.id} | Source: {citation.sourceId}</p>
                        </div>
                      ))}
                    </div>
                  </details>
