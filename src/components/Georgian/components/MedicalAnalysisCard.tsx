...
const getAnalysisType = (instruction: string): { type: string; icon: React.ElementType; color: string } => {
  const lower = instruction.toLowerCase();
  
  if (lower.includes('symptom') || lower.includes('diagnos')) {
    return { type: 'Clinical Assessment', icon: Stethoscope, color: 'from-red-500 to-pink-600' };
  }
  if (lower.includes('medication') || lower.includes('drug') || lower.includes('dosage')) {
    return { type: 'Medication Review', icon: Shield, color: 'from-blue-500 to-indigo-600' };
  }
  if (lower.includes('summary') || lower.includes('summarize')) {
    return { type: 'Clinical Summary', icon: FileText, color: 'from-purple-500 to-violet-600' };
  }
  if (lower.includes('procedure') || lower.includes('treatment')) {
    return { type: 'Treatment Plan', icon: Activity, color: 'from-green-500 to-emerald-600' };
  }
  if (lower.includes('demographic') || lower.includes('history')) {
    return { type: 'Patient History', icon: User, color: 'from-orange-500 to-amber-600' };
  }
  
  return { type: 'General Analysis', icon: Brain, color: 'from-indigo-500 to-purple-600' };
};
...
export const MedicalAnalysisCard: React.FC<MedicalAnalysisCardProps> = ({
  ...
  const analysisType = getAnalysisType(analysis.userInstruction);
