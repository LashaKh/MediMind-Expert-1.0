...
export const PremiumInterpretationResults: React.FC<PremiumInterpretationResultsProps> = ({
  ...
  const getConfidenceLevel = (confidence?: number) => {
    if (!confidence) return { level: 'Unknown', color: 'text-[var(--foreground-secondary)]', gradient: 'from-slate-400 to-slate-500', description: 'Confidence not available' };
    ...
    switch (severity) {
      ...
      default:
        return {
          ...
          textColor: 'text-[var(--cardiology-accent-blue-dark)]'
        };
    }
  };
            ...
            <div className="text-center space-y-4">
              ...
              <div className="flex justify-center">
                <div className="flex space-x-2">
                  ...
                  <div className="w-3 h-3 bg-[var(--cardiology-accent-blue)] rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-red-50 to-red-100 rounded-2xl border border-red-200 shadow-xl">
          <div className="p-8">
            ...
            <div className="text-center space-y-4">
              ...
              <Button className="bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-[var(--foreground)]">
      ...
      <div className={cn("abg-premium", className)}>
        <div className="relative overflow-hidden bg-gradient-to-br from-slate-50 to-slate-100 rounded-2xl border border-[var(--glass-border-light)] shadow-lg">
          <div className="p-8 text-center">
            ...
            <p className="text-[var(--foreground-tertiary)]">No clinical interpretation available</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="space-y-6">
            ...
            <div className="bg-[var(--component-card)] rounded-2xl border border-[var(--glass-border-light)] shadow-xl overflow-hidden">
              <div className="p-6">
                <h3 className="text-xl font-bold text-[var(--foreground)] mb-6 flex items-center gap-2">
                  <TrendingUp className="h-6 w-6 text-[var(--cardiology-accent-blue-dark)]" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="flex items-center gap-4 p-4 bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl border border-blue-200">
                    ...
                    <div>
                      <p className="text-[var(--foreground-tertiary)] text-sm font-medium">{t('abg.interpretation.confidence', 'Confidence')}</p>
                      <p className="text-2xl font-bold text-[var(--foreground)]">
                        {analysisMetrics.confidence ? `${Math.round(analysisMetrics.confidence * 100)}%` : 'N/A'}
                      </p>
                      <p className="text-[var(--foreground-secondary)] text-xs">{getConfidenceLevel(analysisMetrics.confidence).level}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 p-4 bg-gradient-to-br from-orange-50 to-red-50 rounded-xl border border-orange-200">
                    <div className="w-12 h-12 bg-gradient-to-br from-orange-500 to-red-500 rounded-xl flex items-center justify-center">
                      <Activity className="h-6 w-6 text-[var(--foreground)]" />
                    </div>
                    <div>
                      <p className="text-[var(--foreground-tertiary)] text-sm font-medium">{t('abg.interpretation.processingTime', 'Processing Time')}</p>
                      <p className="text-2xl font-bold text-[var(--foreground)]">
                        {analysisMetrics.processingTimeMs ? formatProcessingTime(analysisMetrics.processingTimeMs) : 'N/A'}
                      </p>
                      <p className="text-[var(--foreground-secondary)] text-xs">{t('abg.interpretation.lightningFast', 'Lightning fast')}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 p-4 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl border border-emerald-200">
                    ...
                    <div>
                      <p className="text-[var(--foreground-tertiary)] text-sm font-medium">{t('abg.interpretation.quality', 'Quality')}</p>
                      <p className="text-2xl font-bold text-[var(--foreground)]">{analysisMetrics.quality || 'Premium'}</p>
                      <p className="text-[var(--foreground-secondary)] text-xs">{t('abg.search.aiPowered', 'AI-Powered')}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-6">
            {interpretation.split(/^#{2,3}\s+/gm).filter(section => section.trim()).map((section, index) => {
              ...
              return (
                <div key={index} className="bg-[var(--component-card)] rounded-2xl border border-[var(--glass-border-light)] shadow-xl overflow-hidden">
                  <div className="p-6 cursor-pointer hover:bg-[var(--component-surface-primary)] transition-colors"
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[var(--foreground-tertiary)]">Clinical findings and analysis</p>
                        </div>
                      </div>
                        ...
                        <div 
                          ...
                          dangerouslySetInnerHTML={{
                            __html: content
                              ...
                              .replace(/•?\s*\[RED\]\s*([^:]+):/gi, 
                                '<div class="flex items-center gap-3 mb-3 p-3 rounded-lg border bg-red-100 text-red-800 border-red-200"><div class="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center flex-shrink-0"><span class="text-xs font-bold text-[var(--foreground)]">!</span></div><strong class="font-semibold text-red-800">$1:</strong></div>')
                              .replace(/•?\s*\[YELLOW\]\s*([^:]+):/gi, 
                                '<div class="flex items-center gap-3 mb-3 p-3 rounded-lg border bg-yellow-100 text-yellow-800 border-yellow-200"><div class="w-5 h-5 rounded-full bg-yellow-500 flex items-center justify-center flex-shrink-0"><span class="text-xs font-bold text-[var(--foreground)]">!</span></div><strong class="font-semibold text-yellow-800">$1:</strong></div>')
                              .replace(/•?\s*\[GREEN\]\s*([^:]+):/gi, 
                                '<div class="flex items-center gap-3 mb-3 p-3 rounded-lg border bg-emerald-100 text-emerald-800 border-emerald-200"><div class="w-5 h-5 rounded-full bg-emerald-500 flex items-center justify-center flex-shrink-0"><span class="text-xs font-bold text-[var(--foreground)]">✓</span></div><strong class="font-semibold text-emerald-800">$1:</strong></div>')
                              ...
                              .replace(/\*\*([0-9\.]+\s*(?:mmHg|mmol\/L|g\/dL|kPa|%))\*\*/g, 
                                '<span class="inline-flex items-center gap-2 px-2 py-1 bg-[var(--component-surface-secondary)] rounded-md font-mono text-sm font-semibold text-[var(--foreground)]">$1</span>')
                              // Format ion values like Na⁺: 124 mmol/L, Ca²⁺: 1.06 mmol/L
                              .replace(/([A-Za-z]+[⁺²⁻]+):\s*([0-9\.]+\s*mmol\/L)/g, 
                                '<span class="inline-flex items-center gap-2 px-2 py-1 bg-[var(--cardiology-accent-blue-light)] rounded-md font-mono text-sm font-semibold text-blue-800">$1: $2</span>')
                              // Format regular medical values without asterisks
                              .replace(/\b([0-9\.]+\s*(?:mmHg|mmol\/L|g\/dL|kPa|%))\b/g, 
                                '<span class="inline-flex items-center gap-1 px-2 py-1 bg-[var(--component-surface-primary)] rounded text-sm font-mono text-[var(--foreground)]">$1</span>')
                              ...
                              .replace(/^-\s+\*\*(.*?)\*\*:\s*(.*?)$/gm, 
                                '<div class="flex items-start gap-3 mb-3 p-3 bg-[var(--component-card)] rounded-lg border border-[var(--glass-border-light)]"><div class="w-2 h-2 bg-[var(--cardiology-accent-blue)] rounded-full mt-2 flex-shrink-0"></div><div><strong class="font-semibold text-[var(--foreground)]">$1:</strong> <span class="text-[var(--foreground)]">$2</span></div></div>')
                              ...
                              .replace(/^-\s+(.*?)$/gm, 
                                '<div class="flex items-start gap-3 mb-2"><div class="w-2 h-2 bg-[var(--muted)] rounded-full mt-2 flex-shrink-0"></div><span class="text-[var(--foreground)]">$1</span></div>')
                              // Handle any remaining ### headers that might be within content
                              .replace(/^###\s+(.*?)$/gm, '<h4 class="text-lg font-bold text-[var(--foreground)] mb-3 mt-6">$1</h4>')
                              // Format paragraphs properly
                              .replace(/\n\n+/g, '</p><p class="mb-4 text-[var(--foreground)] leading-relaxed">')
                              .replace(/^/, '<p class="mb-4 text-[var(--foreground)] leading-relaxed">')
                              .replace(/$/, '</p>')
                              // Clean up empty paragraphs
                              .replace(/<p class="mb-4 text-[var(--foreground)] leading-relaxed"><\/p>/g, '')
                              // Clean up paragraphs that only contain div elements
                              .replace(/<p class="mb-4 text-[var(--foreground)] leading-relaxed">(<div.*?<\/div>)<\/p>/g, '$1')
    ...
    <div className={cn(
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[var(--component-card)] rounded-2xl border border-[var(--glass-border-light)] shadow-xl overflow-hidden">
            <div 
              className="p-6 cursor-pointer hover:bg-[var(--component-surface-primary)] transition-colors"
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-[var(--foreground)]">Primary Analysis</h3>
                    <p className="text-[var(--foreground-tertiary)]">Core disorder identification and clinical significance</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  {!showHeaderActions && (
              ...
              <div className="px-6 pb-6 space-y-4 animate-in slide-in-from-top-2">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {interpretation.primaryDisorder && (
                    <div className={cn("p-4 rounded-xl border", severityConfig.bgColor, severityConfig.borderColor)}>
                      <h4 className="font-semibold text-[var(--foreground)] mb-2 flex items-center gap-2">
                        <Lightbulb className="h-5 w-5 text-[var(--cardiology-accent-blue-dark)]" />
                        Primary Disorder
                      </h4>
                  ...
                  {interpretation.clinicalSignificance && (
                    <div className="p-4 rounded-xl border bg-[var(--cardiology-accent-blue-light)] border-blue-200">
                      <h4 className="font-semibold text-[var(--foreground)] mb-2 flex items-center gap-2">
                        <Info className="h-5 w-5 text-[var(--cardiology-accent-blue-dark)]" />
                        Clinical Significance
                      </h4>
                      <p className="text-[var(--cardiology-accent-blue-dark)]">{interpretation.clinicalSignificance}</p>
                    </div>
                  )}
                </div>
                ...
                {interpretation.severity && (
                  <div className={cn("p-6 rounded-xl border", severityConfig.bgColor, severityConfig.borderColor)}>
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <h4 className="font-semibold text-[var(--foreground)]">Severity Assessment</h4>
                    </div>
                    <div className="flex items-center gap-4">
                      <Badge className={cn(
                      ...
                      <span className="text-[var(--foreground-tertiary)]">Requires appropriate clinical attention</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-[var(--foreground)]">Physiological Systems</h3>
                      <p className="text-[var(--foreground-tertiary)]">Oxygenation, ventilation, and acid-base status</p>
                    </div>
                  </div>
                  <ArrowRight className={cn(
                    "h-5 w-5 text-slate-400 transition-transform",
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    {interpretation.ventilationStatus && (
                      <div className="p-4 rounded-xl border bg-[var(--cardiology-accent-blue-light)] border-blue-200">
                        ...
                        <p className="text-[var(--cardiology-accent-blue-dark)] font-medium">{interpretation.ventilationStatus}</p>
                      </div>
                    )}
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-[var(--foreground)]">Additional Findings</h3>
                      <p className="text-[var(--foreground-tertiary)]">Secondary observations and clinical correlations</p>
                    </div>
                  </div>
                  <ArrowRight className={cn(
                    "h-5 w-5 text-slate-400 transition-transform",
                ...
                <div className="px-6 pb-6 animate-in slide-in-from-top-2">
                  <div className="space-y-3">
                    {interpretation.additionalFindings.map((finding, index) => (
                      <div key={index} className="flex items-start gap-3 p-4 bg-[var(--component-surface-primary)] rounded-xl border border-[var(--glass-border-light)]">
                        <div className="w-6 h-6 bg-[var(--cardiology-accent-blue-light)] rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                          <span className="text-xs font-semibold text-[var(--cardiology-accent-blue-dark)]">{index + 1}</span>
                        </div>
                        <p className="text-[var(--foreground)] leading-relaxed">{finding}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-xl flex items-center justify-center">
                      <BookOpen className="h-6 w-6 text-[var(--foreground)]" />
                    </div>
                    <div>
                      <h3 className="text-xl font-bold text-[var(--foreground)]">Clinical Recommendations</h3>
                      <p className="text-[var(--foreground-tertiary)]">Suggested actions and monitoring considerations</p>
                    </div>
                  </div>
                  <ArrowRight className={cn(
                    "h-5 w-5 text-slate-400 transition-transform",
