...
async function checkAllTranslations() {
  ...
  for (const namespace of namespaces) {
    ...
    const enData = await loadNamespace('en', namespace);
    const kaData = await loadNamespace('ka', namespace);
    const ruData = await loadNamespace('ru', namespace);
...
// სკრიპტის გაშვება
checkAllTranslations().then(exitCode => {
